  ...
  return (
    <div className="overflow-x-auto rounded-xl border border-gray-100 bg-white shadow-sm">
      <table className="w-full text-left text-sm">
        <thead className="bg-gray-50 text-xs font-medium text-gray-400 uppercase">
          <tr>
          ...
          {transactions.map((tx) => {
            ...
            return (
              <tr key={tx.hash} className="transition-colors hover:bg-gray-50/50">
                <td className="max-w-[120px] truncate px-6 py-4 font-mono text-blue-600">
                  {tx.hash}
                </td>
                ...
                <td className="px-6 py-4 font-bold">
                  {isOut ? '-' : '+'}
                  {parseFloat(tx.value).toFixed(6)} {tx.asset}
                </td>
