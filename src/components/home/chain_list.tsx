...
export default function ChainList() {
  ...
  if (error) {
    return (
      <section className="mx-[20px] w-full max-w-7xl px-4 py-8 pb-32">
        <p className="mb-4 text-3xl font-bold">多鏈數據，一站解決</p>
        <div className="rounded-xl border border-red-900/50 bg-red-900/10 p-6 text-center text-red-400">
          {error}
        </div>
      </section>
    );
    ...
    <section className="mx-[20px] w-full max-w-7xl px-4 py-8 pb-32">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="flex flex-col items-center justify-center rounded-xl border border-gray-800 bg-[#111] p-6 text-center">
          ...
          <Link
            href="/all_chain"
            className="flex w-full items-center justify-center gap-2 rounded-full bg-[#5841D8] py-3 text-sm font-bold text-white transition hover:bg-[#4b36c0]"
          >
            查看更多
          </Link>
        </div>
