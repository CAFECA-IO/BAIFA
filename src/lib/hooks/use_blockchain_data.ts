  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const formattedTransactions: ITransaction[] = (
        ...
        // 3. 處理方法名稱描述 (利用你現有的 getMethodDescription)
        const method = getMethodDescription(tx.input);

        return {
          hash: tx.hash,
          method: method === '0x' ? 'Transfer' : method, // 如果是 0x 通常是純轉帳,
          ...
          from: tx.from ?? '-',
          to: tx.to ?? 'Contract Creation', // to 為 null 代表部署合約
