...
export const createCISFirstPart = (
  thead: string[],
  endedDate: string,
  dataA: IComprehensiveIncomeNeo | undefined,
  dataB: IComprehensiveIncomeNeo | undefined
) => {
  const subThead = [
...
export const createCISLastPart = (
  ...
  const b026ItemTitle =
    +dataA.otherGainLosses.details.investmentGains >= 0
      ? 'B026 Investment gains'
      : 'B026 Investment losses';
  const b027ItemTitle =
    +dataA.otherGainLosses.details.forexGains >= 0 ? 'B027 Forex gains' : 'B027 Forex losses';
  const b028ItemTitle =
    +dataA.otherGainLosses.details.cryptocurrencyGains.weightedAverageCost >= 0
      ? 'B028 Cryptocurrency gains'
      : 'B028 Cryptocurrency losses';
  const b033ItemTitle =
    +dataA.otherGainLosses.weightedAverageCost >= 0
  ...
  return {
    tbody: [
      ...
      },
      {
        ...
        rowData: [
          b026ItemTitle,
          `$ ${roundToDecimal(+dataA.otherGainLosses.details.investmentGains, 2)}`,
          `$ ${roundToDecimal(+dataB.otherGainLosses.details.investmentGains, 2)}`,
        ],
      },
      {
        rowType: RowType.bookkeeping,
        rowData: [
          b027ItemTitle,
          `${roundToDecimal(+dataA.otherGainLosses.details.forexGains, 2)}`,
          `${roundToDecimal(+dataB.otherGainLosses.details.forexGains, 2)}`,
        ],
      },
      {
        rowType: RowType.bookkeeping,
        rowData: [
          b028ItemTitle,
          `${roundToDecimal(
            +dataA.otherGainLosses.details.cryptocurrencyGains.weightedAverageCost,
            2
          )}`,
          `${roundToDecimal(
            +dataB.otherGainLosses.details.cryptocurrencyGains.weightedAverageCost,
            2
          )}`,
        ],
      },
      {
        ...
        rowData: [
          b033ItemTitle,
          `${roundToDecimal(+dataA.otherGainLosses.weightedAverageCost, 2)}`,
          `${roundToDecimal(+dataB.otherGainLosses.weightedAverageCost, 2)}`,
        ],
      },
      {
        rowType: RowType.foot,
...
export const createRevenueChangeTable = (
  endedDate: string,
  TheadDates: string[],
  thisYearData: IComprehensiveIncomeNeo | undefined,
  lastYearData: IComprehensiveIncomeNeo | undefined
) => {
